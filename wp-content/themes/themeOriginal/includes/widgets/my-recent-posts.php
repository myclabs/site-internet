...
class MY_PostWidget extends WP_Widget {
  ...
  /** @see WP_Widget::widget */
              ...
              <?php echo $before_widget; ?>
                  <?php if ( $title )
                        echo $before_title . $title . $after_title; ?>
								...
								<li>
								<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to', 'theme1272');?> <?php the_title_attribute(); ?>">
								<span class="smalltext"><?php the_time('F j, Y') ?></span>
								<h4><?php the_title(); ?></h4>
								<?php $excerpt = get_the_excerpt(); echo my_string_limit_words($excerpt,10);?><small class="more"><?php _e('more', 'theme1272');?> &raquo;</small></a>
								</li>
								<?php endwhile; ?>
								</ul>
								<?php endif; ?>
    ...
    function form($instance) {				
      ...
      <p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category ID:', 'theme1272'); ?> <input class="widefat" id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" type="text" value="<?php echo $category; ?>" /></label></p>
